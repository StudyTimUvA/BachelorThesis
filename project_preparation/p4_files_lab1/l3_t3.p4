...
control MyEgress(inout headers_t hdr, inout user_metadata_t umd, inout standard_metadata_t smd) {
    ...
    action base_action() {
        // if the multicast bit is set, drop the packet
        if (hdr.ipv4.dst[4:0] == 0xe) {
            drop();
        }
        if (hdr.ipv6.dst[8:0] == 0xff) {
            drop();
        }

        // TODO: add header definitions, and add the main header, and emit!
        // Check tcp port == 23
        if (hdr.ipv4.proto == 0x06) {
            if (hdr.ethernet.dst == 0x0017) {
                drop();
            }
        }
        // Check udp port == 60
        if (hdr.ipv4.proto == 0x11) {
            if (hdr.ethernet.dst == 0x003c) {
                drop();
            }
        }
    }
